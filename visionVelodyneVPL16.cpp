...
CVisionVelodyneVPL16::CVisionVelodyneVPL16(int scriptHandle,const int visionSensorHandles[4],double frequency,int options,double pointSize,double coloringDistances[2],double scalingFactor,int newPointCloudHandle)
{
...
bool CVisionVelodyneVPL16::handle(double dt,std::vector<double>& pts,bool getAbsPts,std::vector<unsigned char>& retCols)
{
    pts.clear();
    retCols.clear();
    bool retVal=true;
...
void CVisionVelodyneVPL16::_removePointsBetween(double lowAngle,double range)
{
...
void CVisionVelodyneVPL16::_getColorFromIntensity(double intensity,unsigned char col[3])
{
    if (intensity>1.0f)
        intensity=1.0f;
    if (intensity<0.0f)
